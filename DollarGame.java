...
public class DollarGame {
	...
	int edges;
		...
		//If the edgelist is null, the user entered that they quit.
		if (edgeList == null)
		{
		...
		//If this returned null, then we quit.
		if (vertexList == null)
		{
			return;
		}
		...
		//At this point, we have a list of edges, a list of vertices, and all vertices have dollars initalized.
		//Now we can activate and play the game itself.
	 ...
	 * 
	 * 
	 * Returns the user-defined edges as strings.
	 * 
	 * User may return "Q" or "q" to return a null list, which signifies the user is quitting.
	 * 
	 */
	 ...
	 * 
	 * 
	 * This function returns true if the string currentEdge qualifies as a valid edge for the game.
	 * 
	...
	public static Vertex[] initializeVertices(int vertexCount)
	{
		...
		//currentChar: Sets the vertex as the current character.  Starts at "a"
		char currentChar = 'a';
		
		
		//isValid:  For getting a valid input for dollars.
		boolean isValid = false;
		...
		{
			currentChar = (char)(STARTING_CHAR + x);
			
			vertexList[x] = new Vertex(currentChar);
			
			
			System.out.println("Please enter the number of dollars for Vertex " + currentChar);
			
			do
			{
				String dollarString = reader.nextLine();
				
				if(dollarString.equals("q") || dollarString.equals("Q"))
				{
					return null;
				}
				...
				try
				{
					...
					//The user entered a valid integer, thus we set it.
					isValid = true;
					
					vertexList[x].setDollars(dollarInt);
				}
				catch(NumberFormatException e)
				{
					//The user did not enter "Q", "q", or a valid integer.
					System.out.println("Error.  Invalid input.  Please enter a whole number for the vetex's dollars.");
	...
	public static void playGame(Vertex[] vertexList, String[] edgeList)
	{
